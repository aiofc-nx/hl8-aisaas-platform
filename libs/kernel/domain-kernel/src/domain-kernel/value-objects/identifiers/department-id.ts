...
export class DepartmentId {
   ...
   * @description 首先比较组织ID，然后比较值
   */
  public static compare(a: DepartmentId, b: DepartmentId): number {
    const orgCompare = OrganizationId.compare(
      a._organizationId,
      b._organizationId,
    );
    if (orgCompare !== 0) {
   ...
   * 获取标识符的哈希值
   * @returns 哈希值
   * @description 基于值和组织ID计算哈希值
   */
  public hashCode(): number {
    let hash = 0;
    const combined = `${this._organizationId.value}:${this._value}`;
