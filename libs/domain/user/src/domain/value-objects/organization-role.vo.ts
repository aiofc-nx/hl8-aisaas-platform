/**
 * @fileoverview 组织角色值对象
 * @description 封装组织角色的业务规则
 */
 ...
 * ```
 */
export class OrganizationRole extends ValueObject<string> {
  /**
   * 构造函数
   * @param value 角色值
   */
  ...
  protected validateValue(value: string): void {
    ...
    if (trimmed.length < 1 || trimmed.length > 50) {
      throw new Error("组织角色长度必须在 1-50 字符之间");
    }
  }
  ...
  /**
   * 克隆组织角色值对象
   * @returns 新的 OrganizationRole 实例
   */
  clone(): OrganizationRole {
    return new OrganizationRole(this._value);
  }
