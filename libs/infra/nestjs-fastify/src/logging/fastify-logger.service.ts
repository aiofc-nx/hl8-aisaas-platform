...
import type { MetricsService } from "../performance/metrics/metrics.service.js";
...
export class FastifyLoggerService implements NestLoggerService {
   ...
   * ```
   */
  child(context: Record<string, unknown>): FastifyLoggerService {
    // 使用 Pino child() 方法创建子日志器
    const childPino = this.pinoLogger.child(context);

    // 创建新的 FastifyLoggerService 实例，复用配置和 metricsService
  ...
  private writeLogWithContext(
    ...
    const startTime = this.config?.performance?.trackLogWriteTime
      ? Date.now()
      : undefined;
    ...
    try {
      ...
      // 记录性能指标（如果启用）
      if (this.config?.performance?.enabled && this.metricsService) {
        const duration = startTime ? Date.now() - startTime : 0;
        this.recordLogMetrics(level, duration, context);
      }
    } catch (error) {
      // 错误处理
  ...
  private recordLogMetrics(
    ...
    try {
      // 通过 MetricsService 访问 PrometheusService
      // 注意：MetricsService 封装了 PrometheusService，需要通过私有属性访问
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const prometheusService = (this.metricsService as any).prometheusService;
      ...
      // 记录写入耗时（秒）
      if (prometheusService.logWriteDuration) {
        prometheusService.logWriteDuration.observe(labels, duration / 1000);
      }

      // 记录写入总数
      if (prometheusService.logWriteTotal) {
        prometheusService.logWriteTotal.inc(labels);
      }

      // 记录日志大小（字节）
      if (prometheusService.logSizeBytes) {
        const logSize = JSON.stringify(context).length;
        prometheusService.logSizeBytes.observe(labels, logSize);
      }
    } catch {
      // 指标记录失败不应影响日志写入
      // 静默处理
  ...
  private handleLogError(
    ...
    if (this.metricsService) {
      try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const prometheusService = (this.metricsService as any)
          .prometheusService;
        if (prometheusService?.logWriteErrorsTotal) {
          prometheusService.logWriteErrorsTotal.inc();
        }
      } catch {
        // 指标记录失败，静默处理
      }
