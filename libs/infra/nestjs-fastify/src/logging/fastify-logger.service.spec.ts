/**
 * FastifyLoggerService 单元测试
 *
 * @description 测试 FastifyLoggerService 的功能，包括日志级别对齐和子日志器
 */
...
describe("FastifyLoggerService", () => {
  ...
  beforeEach(() => {
    ...
    loggerService = new FastifyLoggerService(pinoLogger, config);
  });
  ...
  describe("子日志器支持", () => {
    it("应该支持创建子日志器", () => {
      ...
      expect(childLogger).toBeInstanceOf(FastifyLoggerService);
  ...
  describe("上下文注入", () => {
    ...
    it("应该在上下文禁用时不注入上下文", () => {
      ...
      const loggerWithoutContext = new FastifyLoggerService(pinoLogger, config);
  ...
  describe("脱敏功能", () => {
    ...
    it("应该在脱敏禁用时不脱敏", () => {
      ...
      const loggerWithoutSanitizer = new FastifyLoggerService(
        pinoLogger,
        config,
      );
  ...
  describe("结构化上下文支持", () => {
    ...
    it("应该支持所有日志级别的结构化上下文", () => {
      ...
      const errorSpy = jest.spyOn(pinoLogger, "error");
      loggerService.error("错误日志", context);
      expect(errorSpy).toHaveBeenCalled();
      errorSpy.mockRestore();
    });
  });
  ...
  describe("性能监控", () => {
    ...
    it("应该记录日志写入耗时指标", () => {
      ...
      const loggerWithMetrics = new FastifyLoggerService(
        pinoLogger,
        config,
        metricsService,
      );
      ...
      const observeSpy = jest.spyOn(
        prometheusService.logWriteDuration,
        "observe",
      );

      loggerWithMetrics.log("测试日志");

      expect(observeSpy).toHaveBeenCalled();
      const callArgs = observeSpy.mock.calls[0];
      if (callArgs) {
        const labels = callArgs[0] as Record<string, string>;
    ...
    it("应该记录日志写入总数指标", () => {
      ...
      const incSpy = jest.spyOn(prometheusService.logWriteTotal, "inc");

      loggerWithMetrics.log("测试日志");
      loggerWithMetrics.warn("测试警告");
      loggerWithMetrics.error("测试错误");

      expect(incSpy).toHaveBeenCalledTimes(3);
      const callArgs = incSpy.mock.calls;
      expect(callArgs[0][0].level).toBe("info");
      expect(callArgs[1][0].level).toBe("warn");
      expect(callArgs[2][0].level).toBe("error");
    ...
    it("应该记录日志大小指标", () => {
      ...
      const observeSpy = jest.spyOn(prometheusService.logSizeBytes, "observe");
      ...
      if (callArgs) {
        const labels = callArgs[0] as Record<string, string>;
        const size = callArgs[1] as number;
        expect(labels.level).toBe("info");
    ...
    it("应该在性能监控禁用时不记录指标", () => {
      ...
      const loggerWithoutMetrics = new FastifyLoggerService(
        pinoLogger,
        config,
        metricsService,
      );
    ...
    it("应该在 MetricsService 不可用时正常写入日志", () => {
      config.performance = {
        enabled: true,
        trackLogWriteTime: true,
      };
    ...
    it("应该记录不同级别的日志指标", () => {
      ...
      loggerWithMetrics.verbose("详细日志");
      loggerWithMetrics.debug("调试日志");
      loggerWithMetrics.log("信息日志");
      loggerWithMetrics.warn("警告日志");
      loggerWithMetrics.error("错误日志");

      expect(incSpy).toHaveBeenCalledTimes(5);
      const callArgs = incSpy.mock.calls;
      expect(callArgs[0][0].level).toBe("trace");
      expect(callArgs[1][0].level).toBe("debug");
      expect(callArgs[2][0].level).toBe("info");
      expect(callArgs[3][0].level).toBe("warn");
      expect(callArgs[4][0].level).toBe("error");

      incSpy.mockRestore();
    });

    it("应该在指标记录失败时不影响日志写入", () => {
      config.performance = {
        enabled: true,
        trackLogWriteTime: true,
      };
      ...
      const observeSpy = jest
        .spyOn(prometheusService.logWriteDuration, "observe")
        .mockImplementation(() => {
