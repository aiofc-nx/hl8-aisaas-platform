/**
 * Fastify 日志模块
 *
 * @description 零配置的 Fastify 日志模块，自动使用 Fastify 内置的 Pino
 *
 * ## 特性
 * - ⚡ 零开销（复用 Fastify Pino）
 * - 🔍 便于日志分析和审计
 * - 🔧 支持配置化（可选）
 *
 * @since 0.1.0
 */

import { ConfigValidator } from "@hl8/config";
import { DynamicModule, Global, Module } from "@nestjs/common";
import { HttpAdapterHost } from "@nestjs/core";
import pino from "pino";
import { LoggingConfig } from "../config/logging.config.js";
import { FastifyLoggerService } from "./fastify-logger.service.js";
import { registerRequestContextHook } from "./hooks/request-context.hook.js";
import { MetricsService } from "../performance/metrics/metrics.service.js";

/**
 * 日志模块选项
 */
export interface FastifyLoggingModuleOptions {
  /** 日志配置 */
  config?: Partial<LoggingConfig>;
}

@Global()
@Module({})
export class FastifyLoggingModule {
  /**
   * 注册日志模块
   *
   * @description 创建并配置日志模块
   *
   * ## 业务规则
   * - 优先使用 Fastify 的 Pino 实例（零开销）
   * - 支持配置验证
   *
   * @param {FastifyLoggingModuleOptions} options - 日志配置选项
   * @returns {DynamicModule} 动态模块
   *
   * @example
   * ```typescript
   * @Module({
   *   imports: [
   *     FastifyLoggingModule.forRoot({
   *       config: {
   *         level: 'debug',
   *         prettyPrint: true,
   *       }
   *     })
   *   ]
   * })
   * export class AppModule {}
   * ```
   */
  static forRoot(options?: FastifyLoggingModuleOptions): DynamicModule {
    // 验证和规范化配置
    const loggingConfig = options?.config
      ? ConfigValidator.validate(LoggingConfig, {
          ...new LoggingConfig(),
          ...options.config,
        })
      : new LoggingConfig();

    return {
      module: FastifyLoggingModule,
      global: true,
      providers: [
        // 提供配置实例
        {
          provide: LoggingConfig,
          useValue: loggingConfig,
        },
        // 提供日志服务
        {
          provide: FastifyLoggerService,
          useFactory: (
            httpAdapterHost: HttpAdapterHost,
            config: LoggingConfig,
            metricsService?: MetricsService,
          ) => {
            // 如果日志被禁用，返回静默日志实例
            if (!config.enabled) {
              return new FastifyLoggerService(
                pino({ level: "silent" }),
                config,
                metricsService,
              );
            }

            // 获取 Fastify 实例
            const fastifyInstance =
              httpAdapterHost?.httpAdapter?.getInstance?.();

            // 如果 Fastify 实例不可用（例如在测试环境中），创建一个独立的 Pino 实例
            if (!fastifyInstance?.log) {
              // 在测试环境中，创建一个独立的 Pino 实例
              // 注意：这不会注册请求上下文钩子，因为需要 Fastify 实例
              const testLogger = pino({
                level: loggingConfig.level || "info",
                ...(loggingConfig.prettyPrint && {
                  transport: {
                    target: "pino-pretty",
                  },
                }),
              });

              return new FastifyLoggerService(
                testLogger,
                loggingConfig,
                metricsService,
              );
            }

            // 注册请求上下文钩子（如果启用）
            if (loggingConfig.context?.enabled !== false) {
              registerRequestContextHook(fastifyInstance, {
                config: loggingConfig.context,
              });
            }

            // 使用 Fastify 的 Pino 实例（零开销）
            return new FastifyLoggerService(
              fastifyInstance.log,
              loggingConfig,
              metricsService,
            );
          },
          inject: [
            HttpAdapterHost,
            LoggingConfig,
            { token: MetricsService, optional: true },
          ],
        },
      ],
      exports: [FastifyLoggerService, LoggingConfig],
    };
  }
}
